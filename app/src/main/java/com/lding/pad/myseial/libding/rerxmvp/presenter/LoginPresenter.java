...
import com.lding.pad.myseial.libding.rerxmvp.model.LoginModel;
 ...
 */
public class LoginPresenter implements interfaceUtilsAll.ILoginPresenter {
    private interfaceUtilsAll.ILoginView mLoginView;
    private interfaceUtilsAll.ILoginModel mLoginModel;


    public LoginPresenter(interfaceUtilsAll.ILoginView mLoginView) {
    ...
    public void login() {
        ...
        final int i = 0;
        mLoginModel.login(mLoginView.getUserLoginInfo(), new OnHttpCallBack<GetListRsp>() {
            @Override
            public void onSuccessful(GetListRsp getListRsp) {
                mLoginView.hideProgress();//隐藏进度条
                mLoginView.toMain();//跳转到主页面
                mLoginModel.saveUserInfo(mLoginView.getCurContext(), mLoginView.getUserLoginInfo(), getListRsp.toString());//保存用户数据
